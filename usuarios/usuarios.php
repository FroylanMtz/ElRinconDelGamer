...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <div class="row">
      <div class="col-xs-2">
        <a href="addMonedasRef.php" class="btn btn-primary btn-block boton" style="">Monedas referencia</a>
      </div>
      <div class="col-xs-8">
      </div>
      <div class="col-xs-2">
          <a href="signup.php" class="btn btn-success btn-block boton" style="">Nuevo</a>
      </div>
    </div>
    ...
    $.ajax({
      ...
      success: function (returndata) {
        ...
        if(returndata[0].error==0){
          ...
          rJson = rJson + " <thead> <tr> <th>Nombre</th> <th>Paterno</th> <th>Materno</th> <th>Usuario</th> <th>Rol</th> <th>Editar</th> <th>Eliminar</th> </tr> </thead> ";
          for (i = 0; i < returndata[1].length; i++) {
            rJson = rJson + " <tbody>  <tr id='tr"+returndata[1][i].idUsuario+"'><td>" + returndata[1][i].nombre + "</td><td>" + returndata[1][i].aPaterno + "</td><td>" + returndata[1][i].aMaterno + "</td><td>" +  returndata[1][i].usuario + "</td>";
            if((returndata[1][i].idRol)==1){
              rJson = rJson + "<td>Administrador</td>";
            }else if((returndata[1][i].idRol)==2){
              rJson = rJson + "<td>Gamer</td>";
            }
            rJson = rJson + "<th><a href='signup.php?qqa="+returndata[1][i].idUsuario+"' target='_self' class='btn btn-primary btn-block boton'>EDITAR</a></th>";
            rJson = rJson + "<th><a href='#' target='_self' data-id='"+returndata[1][i].idUsuario+"' class='link_eliminar btn btn-danger btn-block boton'>ELIMINAR</a></th>";
            rJson = rJson + "</tr>";
          }
        ...
        jQuery(document).on('click','.link_eliminar', function() {
          var id=$(this).data("id");
          swal.fire({
            icon: "warning",
                ...
                success: function (returndata) {
                  if(returndata == 1){
                    swal.fire({
                      ...
                      text: "Un usuario fue eliminado con éxito.",
