    ...
    $.ajax({
      ...
      success: function (returndata) {
        ...
        if(returndata[0].error==0){
          ...
          rJson = rJson + " <thead> <tr> <th>Equipo</th> <th>Torneo</th> <th>Fecha</th> <th>Hora</th> </tr> </thead> ";
          for (i = 0; i < returndata[1].length; i++) {
            rJson = rJson + " <tbody>  <tr id='tr"+returndata[1][i].idEquipo+"'><td>" + returndata[1][i].equipo + "</td><td>" + returndata[1][i].titulo + "</td><td>" + returndata[1][i].fecha + "</td><td>" + returndata[1][i].hora + "</td>";
            rJson = rJson + "<th><a href='#' target='_self' data-id='"+returndata[1][i].idEquipo+"' class='link_eliminar btn btn-primary btn-block boton'>Aceptar invitación</a></th>";
            rJson = rJson + "</tr>";
          }
