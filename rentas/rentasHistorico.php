...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <div class="row">
      ...
      <div class="col-xs-2" id="monedasActuales">
      </div>
    </div>
    <div class="contenedortabla">
        <h3><i class="fa fa-users"></i>
        Historial de rentas</h3>
        <div id="resultado"></div>
        <div id="resultadoJson"></div>
    </div>
    <div class="contenedortabla">
        <h3><i class="fa fa-users"></i>
        Historial de cambio de monedas</h3>
        <div id="resultado"></div>
        <div id="resultadoJson2"></div>
    </div>
  </div>
</div>
<!--Archivos JavaScript-->
...
<script type="text/javascript">
  $(document).ready(function(){
    $.ajax({
      ...
      success: function (returndata) {
        ...
        if(returndata[0].error==0){
          ...
          rJson = rJson + " <thead> <tr> <th>Usuario</th> <th>Consola</th> <th>Juego</th> <th>Fecha</th> <th>Hora</th> <th>Promoción</th> </tr> </thead> ";
          for (i = 0; i < returndata[1].length; i++) {
            rJson = rJson + " <tbody>  <tr id='tr"+returndata[1][i].idRenta+"'><td>" + returndata[1][i].usuario + "</td><td>" + returndata[1][i].nombreCon + "</td><td>" + returndata[1][i].nombreJu + "</td><td>" + returndata[1][i].fecha + "</td><td>" + returndata[1][i].hora + "</td>";
            if(returndata[1][i].promocionActiva == 1)
              rJson = rJson + "<td> Promoción 1 </td>";
            else
              rJson = rJson + "<td> Promoción 2 </td>";
            rJson = rJson + "</tr>";
          }
    ...
    $.ajax({
      ...
      success: function (returndata) {
        ...
        if(returndata[0].error==0){
          ...
          $("#monedasActuales").html("<label>Monedas actuales: "+returndata[1][0].totalMonedas)+"</label>";
        }else{
          var rError=returndata[1];
          $("#resultadoJson").html(rError);
        }
    ...
    $.ajax({
      url: 'getHistoricoCambioMonedas_.php',
      type: 'POST',
      dataType:'json',
      async: false,
      cache: false,
      ...
      success: function (returndata) {
        ...
        $("#resultadoJson2").html("");
        if(returndata[0].error==0){
          var rJson = '<table id="example2" class="display" cellspacing="0" width="100%" >';
          rJson = rJson + " <thead> <tr> <th>Usuario</th> <th>Consola</th> <th>Juego</th> <th>Fecha</th> <th>Hora</th> <th>Monedas usadas</th> </tr> </thead> ";
          for (i = 0; i < returndata[1].length; i++) {
            rJson = rJson + " <tbody>  <tr id='tr"+returndata[1][i].idRenta+"'><td>" + returndata[1][i].usuario + "</td><td>" + returndata[1][i].nombreCon + "</td><td>" + returndata[1][i].nombreJu + "</td><td>" + returndata[1][i].fecha + "</td><td>" + returndata[1][i].hora + "</td><td>" + returndata[1][i].monedasUsadas + "</td>";
            rJson = rJson + "</tr>";
          }
          rJson = rJson + " </tbody> </table>";
          $("#resultadoJson2").html(rJson);
          $('#example2').DataTable({
            "language":{
              "lengthMenu":"Mostrar _MENU_ registros por página.",
              "zeroRecords": "Lo sentimos. No se encontraron registros.",
